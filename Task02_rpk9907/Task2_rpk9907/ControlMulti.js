...
*/
			// Face 0: (left side)  
     0.0,  0.0, 2*	sq2, 1.0,		1.0, 1.0,	1.0,	// Node 0
    ...
    -c30, -0.5,  0.0, 1.0, 		0.0,  1.0,  0.0, 	// Node 3
     ...
     c30, -0.5,  0.0, 1.0, 		0.0,  0.0,  1.0, 	// Node 1
		 ...
		 //new 3d part obtuse
		 1, 2, 1, 1, 							1,	1, 	1, // node 1
		 1, 1, 2, 1, 							0,	1, 	0,// node 1
		 2, 1, 1, 1, 							0,	0, 	1, // node 1

		 // new 3d part acute
		 .3, 0, 0, 1, 							1,	1, 	0, // node 1
		 0, 1, 0, 1, 							1,	0, 	1,// node 1
		 0, 0, .05, 1, 							0,	1, 	1, // node 1
  ]);
...
function drawPart1() {
  ...
  g_modelMatrix.scale(1,1,-1);							// convert to left-handed coord sys
  																				// to match WebGL display canvas.
  g_modelMatrix.scale(0.2, 0.3, 0.2);				// Make it smaller.
...
//  OpenGL's 'glGet()' becomes WebGL's 'getParameter()'
  ...
  g_modelMatrix.setTranslate(-0.4,-0.4, 0.0);  // 'set' means DISCARD old matrix,
  						// (drawing axes centered in CVV), and then make new
  						// drawing axes moved to the lower-left corner of CVV. 
  g_modelMatrix.scale(.5,.5,-.5);							// convert to left-handed coord sys
  																				// to match WebGL display canvas.
  g_modelMatrix.scale(0.4, 0.4, 0.4);
  						// if you DON'T scale, tetra goes outside the CVV; clipped!
  g_modelMatrix.rotate(g_angle02, 0, 1, 1);  // Make new drawing axes that
  ...
  g_modelMatrix.scale(1,1,-1);							// convert to left-handed coord sys
	...
	// attach one spiny thing to the bottom
		// DRAW 2 TRIANGLES:		Use this matrix to transform & draw
	//						the different set of vertices stored in our VBO:
  ...
  gl.drawArrays(gl.TRIANGLES, 12,6);
	...
	// ATTACH TRIANGLE 2
 	g_modelMatrix.scale(1/.3,1/.3,1/.3);							// convert to left-handed coord sys
	g_modelMatrix.translate(-c30, 0.5, 0.0)
	g_modelMatrix.translate(-c30, -0.5, 0.0)
	g_modelMatrix.rotate(g_angle02, 1, 0, 0);  // Make new drawing axes that
...
// Attach TRIANGLE 3
	g_modelMatrix.scale(1/.7,1/.7,1/.7);							// convert to left-handed coord sys
	g_modelMatrix.translate(c30, 0.5, 0.0)
...
//  if(angle <  -85.0 && g_angle01Rate < 0) g_angle01Rate = -g_angle01Rate;
  ...
  if(g_angle02 > 180.0 && g_angle02Rate > 0) g_angle02Rate *= -1.0;
